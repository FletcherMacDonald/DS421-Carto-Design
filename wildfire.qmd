...
# Filter for Hawaii with broader bounds
hawaii_fires <- fire_data %>%
  filter(LATITUDE >= 18 & LATITUDE <= 23 & LONGITUDE >= -162 & LONGITUDE <= -154) %>% # Broader Hawaii bounds
  mutate(date = as.Date(ACQ_DATE, format = "%Y-%m-%d")) # Explicit date format

# Check if any data remains after filtering
print("Number of rows after filtering for Hawaii:")
print(nrow(hawaii_fires))

# If no data, provide warning and use sample data
...
ggplot(data = hawaii_fires, aes(x = date, y = fire_count)) +
  geom_line(color = "#D81B60", size = 1.2) + # Red-pink for clarity
  geom_point(color = "#D81B60", size = 3) + # Points for emphasis
  labs(
  ...
  scale_x_date(date_breaks = "1 day", date_labels = "%b %d")yeYes
```
